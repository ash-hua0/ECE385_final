...
module top_level(   input clk,
    ...
    //xadc_wiz_0 my_adc ( .dclk(clk), .daddr_in(8'h13), //read from 0x13 for a
    ...
    tonegen tone( .clk(clk),.rst_in(btnd),
    ...
    pwm (.clk(clk), .rst_in(btnd), .level_in({~vol_out[7],vol_out[6:0]}), .pwm_out(pwm_val));
...
module tonegen(
  ...
  output logic signed [7:0] data_out       // 8-bit PCM data to headphone
    ...
    sine_generator  tone750hz (   .clk(clk), .rst_in(rst_in), 
                                 .step_in(ready_in), .amp_out(tone_750));
    //generate a 440 Hz tone
    // change phase_incr to change output tone! (f*(2^32 -1))/f_s = phase_incr
    sine_generator  #(.PHASE_INCR(32'd39370534)) tone440hz(.clk(clk), .rst_in(rst_in), 
                               .step_in(ready_in), .amp_out(tone_440));                          
    //logic [7:0] data_to_bram;
    //logic [7:0] data_from_bram;
    //logic [15:0] addr;
    //logic wea;
    //  blk_mem_gen_0(.addra(addr), .clka(clk), .dina(data_to_bram), .douta(data_from_bram), 
